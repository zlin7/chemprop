...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
    ...
    return MoleculeDataset([datapoint for datapoint in tqdm(data)
                            ...
                            and all(m.GetNumHeavyAtoms() > 0 for m in datapoint.mol if type(m)!=tuple)
                            and all(m[0].GetNumHeavyAtoms() + m[1].GetNumHeavyAtoms() > 0 for m in datapoint.mol if type(m)==tuple)])
