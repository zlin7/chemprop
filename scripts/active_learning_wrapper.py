...
def active_learning(active_args: ActiveArgs):
    ...
    for i in range(len(active_args.train_sizes)):
        ...
        get_pred_results(active_args=active_args, whole_data=whole_data, iteration=i)
        save_results(active_args=active_args, test_data=test_data, nontest_data=nontest_data, whole_data=whole_data, iteration=i, save_whole_results=True)
...
def get_pred_results(active_args:ActiveArgs, whole_data:MoleculeDataset, iteration:int) -> None:
